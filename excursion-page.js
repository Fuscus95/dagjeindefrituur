(() => {
    ...
    root.innerHTML = `
      <section class="section">
        <div class="container narrow center">
          <h1>Excursie niet gevonden</h1>
          <p>Voor deze excursie is nog geen modulaire detailinhoud toegevoegd.</p>
  ...
  if (excursion.seo) {
    ...
    setMeta('meta[property="og:title"]', "content", excursion.seo.ogTitle || excursion.title);
  ...
  const paragraphs = excursion.detail.paragraphs.map((p) => `<p>${p}</p>`).join("");
  const expectations = excursion.detail.expectations.map((item) => `
    <div class="checkitem"><span>✓</span><div>${item}</div></div>
  `).join("");
  const practical = excursion.detail.practical.map(([label, value]) => `
    <div><dt>${label}</dt><dd>${value}</dd></div>
  `).join("");
  const route = excursion.detail.route.map((step, index) => `
  ...
  const otherExcursions = window.EXCURSIONS
  ...
  root.innerHTML = `
    ...
    <section class="section">
      <div class="container split">
        <div class="content">
          ...
          <h2>${excursion.detail.heading}</h2>
          ${paragraphs}
          <div class="notice">${excursion.detail.notice}</div>
          <h2 style="margin-top:46px;">Wat kun je verwachten?</h2>
          <div class="checklist">${expectations}</div>
        </div>
    ...
    <section class="section">
      <div class="container narrow center">
        <p class="eyebrow dark">Verder veldwerk</p>
        <h2>Ontdek meer vogel- en frituurexcursies</h2>
        <p style="color:var(--muted);font-size:18px;">Vergelijk de snackhabitats en vogelgebieden van onze andere excursies.</p>
        <p>${otherExcursions}</p>
        <a class="btn btn-primary" href="/#excursies">Alle excursies</a>
      </div>
    </section>
